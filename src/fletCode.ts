// Python Flet Code and Guide Data

export const pyFletCode = `import flet as ft
import os
import sqlite3
import hashlib
from datetime import datetime

# --- LÓGICA DE SEGURANÇA E BANCO DE DADOS (SQLite) ---
DB_NAME = "auth.db"
...
def is_setup_done():
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT value FROM config WHERE key='password_hash'")
...
def verify_password(password):
    ...
    conn.close()
    return get_hash(password) == stored_hash
...
def main(page: ft.Page):
    ...
    def update_dashboard_stats():
        ...
        alert_msg = "Estabilidade operacional"
        alert_color = COLOR_CONCLUIDA
        if count_due > 0:
            alert_msg = f"{count_due} Actividades Limites"
            alert_color = COLOR_QUASE_FIM
        elif count_pend > 3:
            alert_msg = "Alta taxa de pendências"
    ...
    def open_change_password(e):
        ...
        def save_new_password(e):
            if len(tf_new_pwd.value) < 4:
                tf_new_pwd.error_text = "Mínimo de 4 caracteres"
                page.update()
                return
            conn = sqlite3.connect(DB_NAME)
            cursor = conn.cursor()
            cursor.execute("UPDATE config SET value=? WHERE key='password_hash'", (get_hash(tf_new_pwd.value),))
            conn.commit()
            conn.close()
            pwd_dialog.open = False
            page.show_snack_bar(ft.SnackBar(ft.Text("✓ Senha alterada com sucesso!", color=COLOR_CONCLUIDA), bgcolor="#142118"))
            page.update()
        ...
        pwd_dialog = ft.AlertDialog(
            ...
            shape=ft.RoundedRectangleBorder(radius=10),
            title=ft.Row([
                ft.Icon(ft.icons.KEY, color="cyan"),
                ft.Text("ALTERAR SENHA", size=16, weight=ft.FontWeight.BOLD, color=COLOR_TEXT_PRIMARY)
            ]),
            content=ft.Container(
                width=350,
                content=ft.Column([
                    ft.Text("Defina uma nova senha para o sistema.", size=12, color="grey"),
                    ft.Divider(height=10, color="transparent"),
                    tf_new_pwd
                ], tight=True, spacing=10)
            ),
            actions=[
                ft.TextButton("Cancelar", on_click=lambda _: close_settings_dialog(pwd_dialog)),
                ft.ElevatedButton("Salvar Senha", bgcolor="cyan", color="black", on_click=save_new_password)
            ],
            actions_alignment=ft.MainAxisAlignment.END
        )
        page.dialog = pwd_dialog
    ...
    def do_setup(e):
        if len(password_field.value) >= 4:
            conn = sqlite3.connect(DB_NAME)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO config (key, value) VALUES (?, ?)", ('password_hash', get_hash(password_field.value)))
            conn.commit()
            conn.close()
            page.clean()
            render_dashboard()
            ...
            password_field.error_text = "Mínimo de 4 caracteres"
            page.update()

    def do_login(e):
        if verify_password(password_field.value):
            page.clean()
            render_dashboard()
            page.update()
        else:
    ...
    def render_auth():
        ...
        auth_container = ft.Container(
            ...
            content=ft.Container(
                ...
                border=ft.border.all(1, "#222222"),
                content=ft.Column([
                    ft.Icon(ft.icons.LOCK_OPEN_OUTLINE if setup_mode else ft.icons.LOCK_OUTLINE, size=50, color=COLOR_PENDENTE),
                    ft.Text("Agenda", size=24, weight=ft.FontWeight.BOLD, color=COLOR_TEXT_PRIMARY),
                    ft.Text("Crie sua senha local." if setup_mode else "Insira sua senha.", size=13, color=COLOR_TEXT_MUTED),
                    ft.Divider(color="transparent", height=10),
                    password_field,
                    ft.ElevatedButton(
                        "SALVAR SENHA" if setup_mode else "ENTRAR",
                        on_click=do_setup if setup_mode else do_login,
                        bgcolor=COLOR_CONCLUIDA,
                        color="#111111",
                        width=320,
                        height=45,
                        style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=6))
...
export const pyInstructionsAndDocs = `### ⚙️ Guia Completo para Executar e Compilar no Windows (.exe)

O Flet permite criar aplicações ricas usando Python, que correm de forma nativa no Desktop (Windows, macOS, Linux). Siga os passos detalhados abaixo para rodar e compilar a sua aplicação.

---

### 1️⃣ Configurando o Ambiente Local

1. **Instalar Python:**
   Instale o Python 3.10 ou superior. Garanta que seleciona a caixa **"Add Python to PATH"** no instalador oficial.

2. **Instalar o Flet:**
   Abra o Terminal ou Prompt de Comando (CMD) e execute o instalador pip:
   \`\`\`bash
   pip install flet
   \`\`\`

3. **Salvar o Arquivo:**
   Crie uma pasta vazia e guarde o código fornecido num ficheiro chamado: **\`main.py\`**.

4. **Rodar para Testar:**
   Execute a aplicação no ambiente de testes local de forma simples:
   \`\`\`bash
   flet run main.py
   \`\`\`
   A janela do app deve-se abrir imediatamente com a estética de Dark Mode Luxo que criamos, permitindo que você altere dinamicamente os nomes dos círculos superiores ao clicar no ícone de Engrenagem (Settings) no Canto Superior Direito.

---

### 2️⃣ Gerando o Executável (.exe) com PyInstaller

Para empacotar tudo em um único ficheiro executável (.exe) de forma profissional, usaremos o **PyInstaller**. Ele empacota o interpretador Python e a interface Flet num pacote estático que roda sem precisar que a máquina destino tenha Python ou Flet instalados.

#### Método Recomendado usando o CLI oficial de Build do Flet:
O Flet possui um utilitário interno inovador que encapsula o PyInstaller garantindo que os pacotes estáticos nativos (C++, Flutter engines) do Flet sejam copiados sem bugs.

\`\`\`bash
# 1. Instalar dependência de publicação
pip install pyinstaller

# 2. Executar o empacotamento desktop nativo (Cria ficheiro único na pasta dist/)
flet pack main.py --name "Aether_Agenda" --icon app_icon.ico
\`\`\`

#### Método Alternativo usando o PyInstaller Diretamente:
Se quiser maior controlo sobre as flags do PyInstaller:

\`\`\`bash
pyinstaller --noconsole --onefile --name="Aether_Agenda" main.py
\`\`\`

#### 💡 Significado das principais Flags:
* **--onefile :** Compacta absolutamente tudo (bibliotecas, python, assets, renderizadores do Flet) num único ficheiro \`.exe\` unificado.
* **--noconsole / --windowed :** Suprime a abertura da janela paralela preta do terminal do Windows. A aplicação abre diretamente como um software comum de interface gráfica.
* **--name="Nome" :** Atribui o nome de sua escolha ao executável gerado.
* **--icon="app_icon.ico" :** (Opcional) Define um ícone estilizado em formato \`.ico\` para o ficheiro final.

---

### 3️⃣ Resolução de Problemas Comuns
* **Erro de antivírus bloqueando:** Ficheiros gerados em modo stand-alone pelo PyInstaller podem ser detectados sob falsos positivos nas primeiras utilizações em sistemas Windows locais. Assine digitalmente o app ou adicione uma exceção de desenvolvedor.
* **Caminho de Arquivos Relativos:** Se anexa ou carrega assets dinâmicos como imagens ou ícones adicionais locais, certifique-se de configurar caminhos relativos ao executável obtidos através de \`os.path.abspath(os.path.dirname(__file__))\` ou empacotando-os com a flag \`--add-data\`.
`;
